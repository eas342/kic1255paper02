...
We find that the Subaru high resolution spectrum best-fits the higher gravity main-sequence star case.}\label{tab:stellObsParams}
\end{deluxetable*}
...
For the blue arm, I find IRAF aperture 1 for the data is Aperture 16, Order 95 in the Subaru HDS Th-Ar atlas.)
...
The median spectrum is visible in Figure \ref{fig:mgTripletBOSZMedianW5000}.

\subsection{Another Reduction}
T Hirano reduced the 2013 and 2015 data and made these available.
A comparison of the E Schlawin (ES) and T Hirano (TH) spectra can be seen.
The 

\begin{figure*}[!hbtp]
\begin{centering}
\includegraphics[width=0.9\textwidth]{images/subaru/es_and_th_comparison.pdf}
\includegraphics[width=0.9\textwidth]{images/subaru/es_and_th_comparison_aperture_22.pdf}
\caption{(Top Panel): Comparison between the T Hirano spectrum (Aperture 24) and E Schlawin one (Aperture 23).
There is likely a mistake in the wavelength calibration.
(Bottom Panel:) Comparing the extracted pixels for Aperture 23 and 24 shows that both recover the Mg Triplet.
The two are shifted likely because they use different traces and apertures.}\label{fig:CaILine}
\end{centering}
\end{figure*}
