...
We observe 5 transits of the system with the 61-inch Kuiper observatory in 2016 and show that they are consistent with photometry from the Kepler spacecraft in 2009-2013.
...
This disfavors the hypothesis that planet disintegration affected previous high resolution spectra results.
We apply PCA to the Kepler long cadence data to understand the modes of disintegration.
...
The discovery of K2-22b \citep{sanchis-ojedak2-22}, WD 1145+017 \citep{vanderburg2015wdDisintegrating}, KOI 2700 \citep{rappaport2014KOI2700} and KIC 3542116 \citep{rappaport2018exocomets} showed that other systems have similar light curves and that several disintegrating planet systems may be useful laboratories for understanding planet evolution and composition.
...
KIC 8462852 may be a family of comets from one or more parent bodies that recently were broken apart \citep{boyajian846,bodman2016kic8462852cometFamily,wright2016familiesOfSolutions}.
...
A radiative-hydrodynamic wind model predicts that the mass loss rate will be a strong function of planet mass and for the present-day mass loss rate of $\sim$ M$_\oplus/Gyr$, the modeled mass is 0.014 $M_\oplus$ \citep{perez-becker}.
...
The SpeX/IRTF spectrum, though affected by systematic noise, confirmed the \citet{croll2014} result that particle sizes are $\gtrsim 0.5\%$ for a pyroxene composition.
...
We construct $f_{SC}(t)$ by phasing the Kepler time series with Equation \ref{eq:ephem}, normalizing by the median, removing a quadratic baseline from each transit, averaging the results and subtracting 1.0 to get the differential flux.
...
We will revisit the statistics of these events in Section \ref{sec:Kepler}.
...
\caption{The principal component eigenvectors scaled by their eigenvalues (Left), shows that the first component largely describes the extinction while the second shows the contributions from forward scattering.
When the eigenvectors are transformed into light curves (Right), it can be seen that the first component corresponds to small dust particle sizes with negligible forward scattering and that the second component corresponds to large dust particle sizes with pronounced forward scattering before and after the transit mid-point.
...
The model using the first 4 principal components shown in Figure \ref{fig:riverPlotsPCA} better matches the data (with more degrees of freedom) and has residuals much closer to white noise than the model that uses the dot product with the average light curve.

Figure \ref{fig:pcaVectorsComp} shows the first 4 eigenvectors as a function of orbital phase.
For the eigenvectors plot, we multiply each eigenvector by its eigenvalue to emphasize the covariances between the principal components and phase grid variables.
...
The first eigenvector largely corresponds to models of small dust grains with more isotropic scattering whereas the second eigenvector corresponds to models of large dust grains with more forward scattering \citep[e.g. models from][]{budaj12}.
The implication of the PCA then is that the dust particle sizes are changing with time.
...
Figure \ref{fig:tserPC} shows the time series of the first two principal components.
...
%We also show the first two principal components as a scatter plot in Figure \ref{fig:pcaVectorsComp}.
%In some applications of PCA, separate populations of behaviors can appear in these scatter plots (such as different modes of planet disintegration) but here there is a continuous distribution points.
%We also color the quiescent periods of very little disintegration activity (discussed in \citet{vanWerkhoven2014} and \citet{croll2015starspots}) and these quiescent periods appear as low values for the first two principal components.
...
%A comparison histogram is shown from the Kepler Secondary eclipses to show the statistical photon noise that explains shy some transit depths appear less than 0.
...
The histogram of secondary eclipse amplitudes from the Kepler light curves is also shown in \ref{fig:histoPhot}, which shows the spread due to photon noise.
...
We obtained ground-based R band photometry from the Kuiper 61-inch observatory to follow up a possible slowdown in disintegration activity observed in 2013 and 2014.
...
Our transit depths from June-July 2016 are instead consistent with the depths from the Kepler observatory from 2009 to 2013, with a KS-test between the two distributions having a p-value of 0.5 that depths are drawn from the same distribution.
...
We find that the first eigenvector corresponds to models with small sized dust grains whereas the second eigenvector corresponds to models with large-sized dust grains with larger forward scattering.
...
We find that the high resolution spectra are consistent with previous photometry and spectroscopy and that the host star is on the main sequence with T=4500 $\pm$ 110 K, log(g) = 4.5, [Fe/H]=-0.1$\pm$0.1, M=0.7$\pm$0.1 R$_\odot$ and M=0.7 $\pm$0.1 M$_\odot$.
