...
\begin{abstract}
KIC 12557548 b is first of a growing class of intriguing disintegrating planet candidates, which lose mass in the form of \added{a metal rich vapor that condenses into} dust particles escaping their surface.
...
After the discovery of K1255, other systems were found to show similar light curve behavior: K2-22b \citep{sanchis-ojedak2-22}, WD 1145+017 \citep{vanderburg2015wdDisintegrating} \replaced{,}{and} KOI 2700 \citep{rappaport2014KOI2700} \deleted{and KIC 3542116 \citep{rappaport2018exocomets}}.
...
\added{In Section \ref{sec:activity}, we examine the activity of the star from the H$\alpha$ line and also discuss how starspots can affect the light curves.}
...
Typical (median) error bars for the time series are shown as red error bars at arbitrary positions.}
...
The second principal components shows one dominant peak at 491 days, \deleted{in addition to the 0.654 orbital period,} indicating possible long term evolution of the dust particle sizes.}\label{fig:ampPeriodogram}
\end{centering}
\end{figure*}
...
\added{We suspect that the non-Gaussian shape of the line spread function is the cause of this discrepancy.}
...
\added{
\section{Spectral Energy Distribution and Parallax}\label{sec:SED}
...
When inverting the Stefan-Boltzman law, we assume an effective temperature of 4400 $\pm$ 70 K from our spectral fitting and a photometric error of 5\%, assumed to be dominated by photometric systematics.
Using this method we find a radius of 0.72 $\pm$ 0.03 R$_\odot$, which is consistent with a main sequence star.

While examining the bolometric flux from all available data, we notice that the WISE band 3 and 4 fluxes at 12 $\mu$m and 22 $\mu$m \citep{wright2010wise} are significantly in excess of a \citet{castelli2004models} stellar model, as shown in Figure \ref{fig:SED}.
This is also true of the disintegrating system K2-22 b.
These infrared fluxes may arise from dust created by the disintegrating planet and pushed out to a larger radius to form a disk or ring.
Further investigation of the infrared excesses should be explored observationally and theoretically.

\begin{figure*}[!hbtp]
\begin{centering}
\includegraphics[width=0.8\textwidth]{kic1255_sed.pdf}
\caption{\added{The spectral Energy distribution from VizieR \citep{ochsenbein2000vizieR}.
The photometry is fit in order to estimate the stellar bolometric flux and derive a stellar radius of 0.72 $\pm$ 0.03 R$_\odot$.
We note that the WISE Band 3 and Band 4 excess at 12$\mu$m and 22 $\mu$m may arise from a dusty ring or disk of material from the planet, warranting further investigation.}}\label{fig:SED}
\end{centering}
\end{figure*}
}.
...
The high resolution spectrum of \shStar\ allows us to measure the H$\alpha$ line, which is an indicator of stellar activity.
Figure \ref{fig:Halpha}, shows the Subaru HDS spectrum for 2015 Aug 28 and 2015 Aug 29.
We note that the strong emission feature at the core of the H$\alpha$ line is visible for one night but not the other, confirming the activity of the star already known from the rotational modulations \citep{kawahara2013starspots}.

\begin{figure*}[!hbtp]
\begin{centering}
\includegraphics[width=0.8\textwidth]{h_alpha_subaru.pdf}
\caption{H$\alpha$ line as measured by Subaru HDS on two adjacent nights in 2015.
The line's core changes noticeably between the two nights.}\label{fig:Halpha}
\end{centering}
\end{figure*}
...
\citet{croll2015starspots} calculate that unocculted spots have a minimal impact on the transit depth variations.
...
Instead, \citet{rackham2018transitSourceEffect} use more realistic simulations of spot covering and light curve modulations and find that the amplitude of variability is proportional to $\sqrt{f_S}$, where $f_S$ is the spot covering fraction.

For the observed amplitude variations of 0.8\% \citep{kawahara2013starspots}, scaling relations for K4 V dwarfs indicate that transit depths of exoplanets can be biased by 10$^{+20}_{-5}$\% depending on the distribution and sizes of spots if the transit chord is completely devoid of spots.\footnote{Benjamin V. Rackham, private communication, 2018}
The large physical size of \sha\, as indicated by a transit duration longer than the time to cross the star, means that spots are likely to be crossed and diminish the transit source effect, so \sha's 30\% transit depth variation would require a worst-case starspot coverage to be explained by un-occulted spots.
Also, the bias in transit depth is different from the transit depth variations, which would be un-affacted by a polar spot.
...
We find that the high resolution spectra are consistent with previous photometry and spectroscopy and that the host star is on the main sequence with T$_{eff}$=4440 $\pm$ 70 K, log(g) = 4.63 $\pm$ 0.12, [Fe/H]=-0.08$\pm$0.09, \replaced{M}{R}=0.69$\pm$0.10 R$_\odot$ and M=0.70 $\pm$0.08 M$_\odot$.
...
While fitting the Spectral Energy Distribution to estimate the bolometric flux, we notice that there is an infrared excess in WISE Bands 3 and 4 that warrants further study.}
