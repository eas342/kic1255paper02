...
\subsection{Transit Depth Fitting}
...
The residual of this model (Figure \ref{fig:riverPlots}) shows significant $\pm$0.2\% deviations from white noise, indicating that the dusty tail surrounding \sha\ shanges in shape with time.
We explore more sophisticated models than the average light curve below that are based on principal component analysis.
However, this average model serves as a useful comparison to fitting lower-precision photometry, such as the ground-based light curves typical photometric scatter is $\sim$ 0.1 to 0.2\% for 20 minute timescales.
...
The quiescent periods discussed in \citet{vanWerkhoven2014} and \citet{croll2015starspots} appear as low values for the first two principal components.
