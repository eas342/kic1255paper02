...
This gas absorption could potentially provide kinematic information on escaping material provided that the sublimated gas has sufficient optical depth.
...
The median spectrum from 2015 has a resolution of $R=60,000$, which is close to the \texttt{specmatch} grid of $R=55,000$, so we do not convolve the input data for \texttt{specmatch}.
%We first convolve the median spectrum from \citet{masuda2018rvKIC1255} with a R=40,000 kernel to better match .
