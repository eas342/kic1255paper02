...
The residual of this model (Figure \ref{fig:riverPlots}) shows significant $\pm$0.2\% deviations from white noise, indicating that the dusty tail surrounding \sha\ changes in shape with time.
...
The 0.2\% residuals with this average model are comparable with the typical photometric scatter of $\sim$ 0.1 to 0.2\% for 20 minute timescales in this work and \citet{schlawin2016kic1255}.
...
\caption{The principal component eigenvectors scaled by the square root of the eigenvalues (Top Left), shows that the first component largely describes the extinction while the second and third show the contributions from forward scattering.
The first two principal components (Bottom) do not isolate any separate populations of transits.
...
For the eigenvectors plot, we weight each eigenvector by the square root of the explained variance of each principal component, as is used in meteorology \citep{richman1986pcaRotation}.
...
The first eigenvector largely corresponds to models of small dust grains with more isotropic scattering whereas the second eigenvector corresponds to models of large dust grains with more forward scattering \citep[e.g.][]{budaj12}.
...
We also color the quiescent periods of very little \sha\ disintegration activity (discussed in \citet{vanWerkhoven2014} and \citet{croll2015starspots}) and these quiescent periods appear as low values for the first two principal components.
This indicates that 
...
\caption{Periodogram of the amplitude time series shown in Figure \ref{fig:tserPC1} show several significant peaks.
Bootstrap error estimates for the 2$\sigma$ and 3$\sigma$ confidence intervals are shown.
The first principal component's peak at 0.654 days is the orbital period (Left).
The PC$_1$ peaks at 153 and 750 days are likely caused by the longer intervals of low disintegration activity.
The second principal components shows one dominant peak at 491 days, in addition to the 0.654 orbital period.
