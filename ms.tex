...
When the eigenvectors are transformed into light curves, 
}\label{fig:pcaVectorsComp}
\end{centering}
\end{figure*}

Figure \ref{fig:pcaVectorsComp} shows the first 4 eigenvectors as a function of orbital phase.
For the eigenvectors plot, we multiply each eigenvector by its eigenvalue, as is used in meteorology where the products of the eigenvalues and eigenvectors are sometimes called Empirical Orthogonal Functions \citep{richman1986pcaRotation}.
...
Figure \ref{fig:tserPC1} shows the time series of the first two principal components.
We highlight the quiescent periods also discussed in \citet{vanWerkhoven2014} and \citet{croll2015starspots}.
The first principal component largely tracks closely with the dot product of the average light curve.
