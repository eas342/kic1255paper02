...
We also evaluate high resolution spectra from the Subaru HDS spectrograph and find spectra consistent with a main-sequence T=4500 $\pm$110 K, main-sequence star.	
\end{abstract}
...
We evaluate the stellar parameters by examining the spectra taken with the High Definition Spectrograph (HDS) spectrograph on the Subaru telescope \citep{noguchi2002hds}.
...
Additionally, there are two set of observations from 2015-08-28 (UT) and 2015-08-29 (UT) that were taken to understand the kinematics of the system \citep{masuda2018rvKIC1255}, with exposure durations of 2400 seconds each.
...
The second set of observations happened after the reaction wheel failure on 2015-05-11\footnote{https://www.nasa.gov/mission\_pages/kepler/news/keplerm-20130521.html} that ended photometry of the main Kepler field, so no simultaneous Kepler photometry was available with the high dispersion spectroscopy.
None of the observations was timed during a primary transit (0.0) or secondary eclipse (0.5).
...
The 2015 data, with a narrow slit (0.3 $\times$ 30 mm, has a sharper PSF and and is easier to trace.
The 2013 data was taken with  an image slicer, which create 3 parallel traces for \shStar.
...
The Mg Triplet (Figure \ref{fig:mgTriplet}) is diagnostic of spectral type and may help resolve some of the conflicts (or variability in interpretation due to planet disintegration) in the literature about the gravity of the host star as listed in Table \ref{tab:stellObsParams}.
...
Again, there is no clear match to either HIGHRES model.
...
K Masuda independently reduced the \shStar\ Subaru data to examine if there could be an orbiting body on a highly eccentric star-grazing orbit that would reveal itself through radial velocity variations \citep{masuda2018rvKIC1255}.
...
\caption{Cleaned Kepler Long-cadence data (Top Left) and model fit using the average light curve (Top Right).
(Bottom) The residuals show significant deviations at phases of -0.5 and 0.05.
...
We start by rescaling the two dimensional grid to have zero mean and a variance of 1.0 along the Transit Number axis, while saving the transformation to return to the original flux units.
This first rescaling step will remove the constants from the transit light curve (such as the planet candidate's transit curve).
Next, we perform PCA on the two dimensional grid to identify eigenvectors as a function of orbital phase and then the principal components as a function of transit number.
...
\begin{centering}
\includegraphics[width=0.49\textwidth]{images/kepler/photometry_riverplot.pdf}
\includegraphics[width=0.49\textwidth]{images/kepler/pca_model_2D.pdf}
\includegraphics[width=0.49\textwidth]{images/kepler/pca_resid_2D.pdf}
\caption{Cleaned Kepler Long-cadence data (Top Left) and model fit using 4 Principal Components (Top Right).
(Bottom) The residuals show smaller residuals than the average model.
...
The 22.9 day peak is consistent with the \citet{kawahara2013starspots} peak that is co-incident with the stellar rotation period.}\label{fig:ampPeriodogram}
\end{centering}
\end{figure*}
