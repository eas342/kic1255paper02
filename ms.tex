...
The second set of observations happened after the reaction wheel failure on 2013-05-11\footnote{\href{https://www.nasa.gov/mission_pages/kepler/news/keplerm-20130521.html}{https://www.nasa.gov/mission\_pages/kepler/news/keplerm-20130521.html}} that ended photometry of the main Kepler field, so no simultaneous Kepler photometry was available with the high dispersion spectroscopy.
None of the observations was timed during a primary transit (0.0) or secondary eclipse (0.5).

\subsection{Data Reduction}
The Subaru data to examine if there could be an orbiting body on a highly eccentric star-grazing orbit that would reveal itself through radial velocity variations \citep{masuda2018rvKIC1255}.
...
\begin{figure*}[!hbtp]
\begin{centering}
\includegraphics[width=0.3\textwidth]{images/kepler/photometry_riverplot.pdf}
\includegraphics[width=0.3\textwidth]{images/kepler/model_riverplot.pdf}
\includegraphics[width=0.3\textwidth]{images/kepler/residual_riverplot.pdf}
...
It is customary with PCA to scale the variables to have zero mean and a variance of 1.0 (ie. along the Transit Number axis) before performing PCA.
However, we elect to keep the original variables without scaling each column in this 2D grid by its standard deviation, since that would magnify the contribution of photon noise in out-of-transit data over the real astrophysical variability of the inner phases (between $\sim$-0.1 to $\sim$ 0.1).
...
\caption{Principal component eigenvectors (Left), shows that the first component largely describes the extinction while the second and third show the contributions from forward scatterg.
The first two principal components (Right) do not isolate any separate populations of transits.
As expected, the quiescent periods show up as low principal component contributions from both of the two eigenvectors (ie. a flatter light curve)}.
\end{centering}
\end{figure*}
...
\software{astropy \citep{astropy2013}, 
	...
	\texttt{iraf},
	\texttt{specmatch} tool \citep{yee2017specMatch} 
