...
We decided to re-visit the stellar parameters by examining the high spectra taken with the High Definition Spectrograph (HDS) spectrograph on the Subaru telescope \citep{noguchi2002hds}.
...
The K Masuda reduction masks all telluric emission features and any outliers, producing cleaner spectra.
...
The BOSZ  \citep{bohlin2017bosz} model database is calculated up to $R = 300,000$ so it can be compared to the measurements without convolving the Subaru spectra.
...
The models are convolved with a Gaussian that has a standard deviation $\sigma_G = 1/57700$  to obtain a resolution of 50,000.
...
Also, we average ll available spectra from the Masuda reduction to increate the signal to noise.
...
This motivates a check on the temperatures, so we compare 3 models all with log(g)=4.5 and 
